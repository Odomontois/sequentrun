...
object Nat {
  ...
  case object Zero extends Nat {
    type Self = Zero
    def elim[F[_]](elim: NatElim[F]): F[Zero] = elim.zero
  }
  final case class Succ[N <: Nat](p: N)(implicit val reg: RegularNat[N]) extends Nat {
    type Self = Succ[N]
    def elim[F[_]](elim: NatElim[F]): F[Succ[N]] = elim.succ(p)
  }
...
trait RegularNat[N <: Nat] {
  def regularize[F[_]](n: N)(f: F[n.Self]): F[N]
}

object RegularNat {
  implicit def nilReg: RegularNat[Zero] = new RegularNat[Zero] {
    def regularize[F[_]](n: Zero)(f: F[Zero]): F[Zero] = f
  }

  implicit def succReg[N](implicit reg: RegularNat[N]): RegularNat[Succ[N]] = new RegularNat[Succ[N]] {
    def regularize[F[_]](n: Succ[N])(f: F[Succ[N]]): F[Succ[N]] = f
  }
}
